...
save.image('zbstm4.RData')
# export file

# Re-upload saved RData file
load(file = "zbstm4.RData")

# Tokenize the texts

# first remove non sinograms (punctuation, symbols)
zbs_p4 <- zbs_p4 %>% mutate(text_clean = str_replace_all(Text, "[^[\\p{Han}]]", " "))

# Remove blank spaces that remain
zbs_p4$text_clean <- str_squish(zbs_p4$text_clean) 
...
# Save file
write_csv(zbs4_tok, "zbs4_tok.csv")

# remove noisy outputs 
zbs4_tok_filt <- zbs4_tok %>% filter(stringr::str_detect(zbs4_tok$Text, "[\\p{Han}]"))

# Add Year to zbs_tok_filt
zbs4_tok_filt <- zbs4_tok_filt %>%
  mutate(Year = as.integer(str_sub(DocId, 5, 8)))
zbs4_tok_filt <- unique(zbs4_tok_filt)
  
# Remove blank spaces that remain
zbs4_tok_filt$Text <- str_squish(zbs4_tok_filt$Text) 

# Save file
write_csv(zbs4_tok, "zbs4_tok.csv")
write_csv(zbs4_tok_filt, "zbs4_tok_filt.csv")
...
zbs_doc <- zbs4_tok_filt %>% select(DocId, Year) 
zbs4_tok2 <- inner_join(zbs_doc, zbs4_tok_filt)
zbs4_tok2 <- unique(zbs4_tok2)
...
zbs4_tok3 <- zbs4_tok3 %>% mutate(Text = str_remove_all(Text, stop_pattern))

# Remove additional stopwords
zbs4_tok3 <- zbs4_tok3 %>% mutate(Text = (str_remove_all(Text, "以及|本會|二馬路|第|一路|個|時間|其中|研究|部分|主要|分析|影响|方面|的|了|在|和|是|為|与|中|日|年|以|等|人|也|上|他們|有|于|之|而|但|由|所|並|到|下|成|及|多|地|者|此|作|被|来|向|就|着|都|具|還|更|起|生|化|其|文|用|大|末|出|海|他|路|名|物|月|王|社|本|时|同|位|期|不|事|工|因|比較|介紹|第|命|性|近代|全面|背景|獨特|原|租|族|两|代|做|先|共|前|北|参|又|字|定|方|著|過|道|部")))


write_csv(zbs4_tok3, "zbs4_tok3.csv")
...
meta <- zbs4_tok3 %>% transmute(DocId, Year)

corpus <- stm::textProcessor(zbs4_tok3$Text,
                             metadata = meta, 
                             stem = FALSE, 
                             wordLengths = c(2, Inf), 
                             verbose = FALSE, 
...
# Removing 20048 of 26273 terms (24184 of 104168 tokens) due to frequency 
# Your corpus now has 2085 documents, 6225 terms and 79984 tokens.
...
# Label topics (word frequencies by score type)
...
ggplot(vizDataFrame5y, aes(x=Year, y=value, fill=variable)) + 
  ...
  labs(title="ZBS in the Shenbao (1919-1926)", 
