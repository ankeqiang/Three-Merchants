...
save.image('zbstm3.RData')
# export file

# Re-upload saved RData file
load(file = "zbstm3.RData")

# Tokenize the texts

# first remove non sinograms (punctuation, symbols)
zbs_p3 <- zbs_p3 %>% mutate(text_clean = str_replace_all(Text, "[^[\\p{Han}]]", " "))

# Remove blank spaces that remain
zbs_p3$text_clean <- str_squish(zbs_p3$text_clean) 
...
# Save file
write_csv(zbs3_tok, "zbs3_tok.csv")

# remove noisy outputs 
zbs3_tok_filt <- zbs3_tok %>% filter(stringr::str_detect(zbs3_tok$Text, "[\\p{Han}]"))

# Select year column in zbs_p3
zbs_p3_year <- zbs_p3 %>% select(DocId, Year)

# Add Year to zbs_tok_filt
zbs3_tok_filt <- left_join(zbs3_tok_filt, zbs_p3_year)
zbs3_tok_filt <- unique(zbs3_tok_filt)

# Remove blank spaces that remain
zbs3_tok_filt$Text <- str_squish(zbs3_tok_filt$Text) 

# Save file
write_csv(zbs3_tok, "zbs3_tok.csv")
write_csv(zbs3_tok_filt, "zbs3_tok_filt.csv")
...
zbs_doc <- zbs3_tok_filt %>% select(DocId, Year) 
zbs3_tok2 <- inner_join(zbs_doc, zbs3_tok_filt)
zbs3_tok2 <- unique(zbs3_tok2)
...
zbs3_tok3 <- zbs3_tok3 %>% mutate(Text = str_remove_all(Text, stop_pattern))

# Remove additional stopwords
zbs3_tok3 <- zbs3_tok3 %>% mutate(Text = (str_remove_all(Text, "以及|本會|二馬路|第|一路|個|時間|其中|研究|部分|主要|分析|影响|方面|的|了|在|和|是|為|与|中|日|年|以|等|人|也|上|他們|有|于|之|而|但|由|所|並|到|下|成|及|多|地|者|此|作|被|来|向|就|着|都|具|還|更|起|生|化|其|文|用|大|末|出|海|他|路|名|物|月|王|社|本|时|同|位|期|不|事|工|因|比較|介紹|第|命|性|近代|全面|背景|獨特|原|租|族|两|代|做|先|共|前|北|参|又|字|定|方|著|過|道|部")))

write_csv(zbs3_tok3, "zbs3_tok3.csv")


#### build the topic model

meta <- zbs3_tok3 %>% transmute(DocId, Year)

corpus <- stm::textProcessor(zbs3_tok3$Text,
                             metadata = meta, 
                             stem = FALSE, 
                             wordLengths = c(2, Inf), 
                             verbose = FALSE, 
...
# Removing 20578 of 26251 terms (24072 of 104311 tokens) due to frequency 
# Your corpus now has 1991 documents, 5673 terms and 80239 tokens.
...
# The Held-Out Likelihood suggests 8 topics is optimal, as the likelihood is maximized here.
# The Residuals suggest fewer topics might be better, as the residuals are lower for 6 topics.
# Semantic Coherence is clearly the highest at 6 topics, indicating strong within-topic word associations.
# The Lower Bound decreases with more topics, suggesting diminishing returns with more than 6 topics.
# Considering all these diagnostics together, it appears that a topic model with 6 topics might be the best trade-off. It has the highest semantic coherence, which is crucial for interpretability and usefulness of the topics. It also has the lowest residuals, indicating a good fit with the observed data.
# However, choosing the best model often depends on a balance of these metrics alongside the interpretability and usefulness of the topics in the context of your specific application. If the topics generated with 8 are more interpretable and meaningful for your needs despite the lower semantic coherence, you may opt for that instead. In practice, it's also common to examine the topics themselves qualitatively to ensure they make sense for the application at hand.
...
write_csv(topicprop5, "zbs3topicprop5.csv")
...
ggplot(vizDataFrame5y, aes(x=Year, y=value, fill=variable)) + 
  ...
  labs(title="ZBS in the Shenbao (1913-1918)", 
