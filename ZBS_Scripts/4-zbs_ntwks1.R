...
save.image('zbsnetsb1.RData')


# Re-upload saved RData file
load(file = "zbsnetsb1.RData")
...
# Creation of the subfiles from filtered_sb1
...
histtext::count_documents('"朱葆三"|"朱佩珍"', "shunpao-revised") %>% 
  ...
  filter (Year >1885 & Year<=1900) %>% 
  ggplot(aes(Year,N)) + geom_col() + 
  labs(title = "朱葆三 in the Shenbao in 1888-1899",
...
write_csv(zbs_nodessb1_Cy, "zbs_nodessb1_Cy.csv")
...
write_csv(edge_sb1, "edge_sb1.csv")
...
plot.igraph(main_component, vertex.size = 3, 
            vertex.label.color = "black", 
            vertex.label.cex = 0.3, 
            main="ZBS Main Component Network in Shenbao (1888-1899)")

# Export for Cytoscape
write_graph(main_component, "zbsnwk1MC.graphml", format = "graphml")
...
write_csv(cutpoints_df, "cutpoints1_df.csv")
...
# Export for Cytoscape
write_graph(main_component, "zbsnwk1cutMC.graphml", format = "graphml")
...
mc1centralities_attributes <- inner_join(node_sb1, mc1centralities_df_labels)

write_csv(mc1centralities_attributes, "mc1centralities.csv")
...
g_bipartite <- graph_from_data_frame(edge_sb1, directed = FALSE)
V(g_bipartite)$type <- ifelse(V(g_bipartite)$name %in% edge_sb1$Source, FALSE, TRUE)
...
write_graph(g_one_mode_person, "zbsnwk1oneM.graphml", format = "graphml")

# Extract Edge List
edge1M_zbs1 <- get.edgelist(g_one_mode_person)
# Convert edge list to a data frame
edge1M_list_df <- data.frame(source = edge1M_zbs1[, 1], target = edge1M_zbs1[, 2])
# Write Edge List to CSV
write.csv(edge1M_list_df, "edge1M_list_df.csv", row.names = FALSE)


# Extract Node Names
node1M_nzbs1 <- V(g_one_mode_person)$name
# Convert node names to a data frame
node1M_nzbs1_df <- data.frame(name = node1M_nzbs1)
# Write Node Names to CSV
write.csv(node1M_nzbs1_df, "node1M_nzbs1_df.csv", row.names = FALSE)



# (Optional) Extract Node Attributes
# Example: Extracting a 'color' attribute
# Adjust according to your actual attributes
if("color" %in% names(V(g_one_mode_person))) {
  node_attributes <- data.frame(name = V(g_one_mode_person)$name, color = V(g_one_mode_person)$color)
  
# Write Node Attributes to CSV
write.csv(node_attributes, "nodes1M_zbs1.csv", row.names = FALSE)
}
